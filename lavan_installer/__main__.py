...
def createApp (connect):
    appname = "lavanuserbot" + str(time() * 1000)[-4:].replace(".", "") + str(random.randint(0,500))
    try:
...
async def oturumacvebotlogolustur (stri, aid, ahash):
    try:
        ...
        await Client.start()
        ms = await Client.send_message('me',LANG['LavanUserBot'])
        ...
        Photo = await Client.upload_file(file='lavan.jpg')
...
if __name__ == "__main__":
    ...
    enc = 'aHR0cHM6Ly9naXRodWIuY29tL0xhdmFuZGVyUHJvamVjdHMvTGF2YW5Vc2VyQm90'
    enc1 = enc.encode('ascii')
    lavan = base64.b64decode(enc1)
    lavander = lavan.decode('ascii')

    if os.path.isdir("./LavanUserBot/"):
        rm_r("./LavanUserBot/")
    repo = Repo.clone_from(lavander,"./LavanUserBot/", branch="master")
    ...
    config['DEFAULT_BIO'] = "✨ @LavanUserBot"
    config['DEFAULT_NAME'] = "Owner"
    ...
    config['TZ'] = TZ
    config['TZ_NUMBER'] = "1"
    config['UPSTREAM_REPO_URL'] = "https://github.com/LavanderProjects/LavanUserBot"
    config['SEVGILI'] = "None"
    config['WARN_LIMIT'] = "3"
